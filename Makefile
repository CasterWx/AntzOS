...
DEL      = del
TREE     = tree /f
Antz_kernel = main/bootpack.obj asmfunc/naskfunc.obj assic/char.set windows/graphic.obj lgdt/set_lgdt.obj interrupt/int.obj io/fifo.obj
...
%.gas : %.c Makefile
	cc1.exe -I lib/ -Os -Wall -quiet -o $*.gas $*.c
...
bootpack.bim : $(Antz_kernel) Makefile
	obj2bim.exe  @lib/haribote.rul out:bootpack.bim stack:3136k map:bootpack.map $(Antz_kernel)
...
antz.img : mbr.bin antz.sys Makefile
	edimg.exe  imgin:lib/fdimg0at.tek wbinimg src:mbr.bin len:512 from:0 to:0  copy from:antz.sys to:@: imgout:antz.img
